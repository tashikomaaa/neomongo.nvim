-- Core primitives that execute MongoDB shell commands and surface the output.
local logger = require("neomongo.log").scope("core")

local M = {}

function M.connect(uri)
    -- Connecting is currently a placeholder that validates shell access.
...
function M.list_dbs(uri)
    -- Use mongosh to list the available databases and echo the output to the user.
    logger("ENTER list_dbs(uri): " .. tostring(uri))
    local cmd =
        string.format('mongosh %s --quiet --eval "db.adminCommand( { listDatabases: 1 } )"', uri)
    logger("List DBs CMD: " .. cmd)
    local result = vim.fn.system(cmd)
    if vim.v.shell_error ~= 0 then
        logger(
            "ERROR list_dbs: shell_error="
...
function M.list_collections(uri, db)
    -- Request the list of collections for a given database. Results are echoed in place.
    ...
    if vim.v.shell_error ~= 0 then
        logger(
            "ERROR list_collections: shell_error="
                .. tostring(vim.v.shell_error)
                .. " | result: "
                .. tostring(result)
        )
...
function M.query(uri, args)
    -- Execute raw MongoDB shell queries through mongosh and stream the result to the user.
